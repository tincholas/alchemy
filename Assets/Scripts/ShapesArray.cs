...
public class ShapesArray
{
    ...
    {
        var g1Shape = go.GetComponent<Shape>();
        int g1Row = g1Shape.Row;
        int g1Column = g1Shape.Column;
        shapes[g1Row, g1Column] = go;
    }
    /// <summary>
    /// Undoes the swap
    /// </summary>
    ...
    public List<GameObject> getMatches()
    {
        List<GameObject> matches = new List<GameObject>();
        //find horizontal matches
        for (int row = 0; row < Constants.Rows; row++)
            for (int column = 0; column < Constants.Columns-2; column++)
            {
                int typeOf1 = shapes[row, column].GetComponent<Shape>().Type;
                int typeOf2 = shapes[row, column+1].GetComponent<Shape>().Type;
                int typeOf3 = shapes[row, column+2].GetComponent<Shape>().Type;
                if (typeOf1 == typeOf2 && typeOf1 == typeOf3)
                {
                    matches.Add(shapes[row, column]);
                    matches.Add(shapes[row, column+1]);
                    matches.Add(shapes[row, column+2]);
                }              
            }
        for (int row = 0; row < Constants.Rows-2; row++)
            for (int column = 0; column < Constants.Columns; column++)
            {
                int typeOf1 = shapes[row, column].GetComponent<Shape>().Type;
                int typeOf2 = shapes[row+1, column].GetComponent<Shape>().Type;
                int typeOf3 = shapes[row+2, column].GetComponent<Shape>().Type;
                if (typeOf1 == typeOf2 && typeOf1 == typeOf3)
                {
                    matches.Add(shapes[row, column]);
                    matches.Add(shapes[row+1, column]);
                    matches.Add(shapes[row+2, column]);
                }
            }
        return matches;
    }
    ...
    /// <param name="columns"></param>
    /// <returns>Info about the GameObjects that were moved</returns>
    public AlteredCandyInfo Collapse()
    {
        AlteredCandyInfo collapseInfo = new AlteredCandyInfo();
    ...
    /// <param name="columns"></param>
    /// <returns>Info about the GameObjects that were moved</returns>
    public AlteredCandyInfo Collapse(int column)
    {
        AlteredCandyInfo collapseInfo = new AlteredCandyInfo();
